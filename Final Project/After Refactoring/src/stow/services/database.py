...
CREATE TABLE IF NOT EXISTS storageSpots
(
    storage_spot INTEGER PRIMARY KEY,
    category TEXT NOT NULL,
    notes TEXT,
    date_stowed TEXT NOT NULL
);
'''
...
class DatabaseManager:
    ...
    def select(self, table_name, criteria=None, order_by=None):
        ...
        if criteria:
            placeholders = [f' {key} {value}' for key, value in criteria.items()]
    ...
    def dbUpdate(self, table_name, data):
        '''
        to UPDATE a SQL table:
        UPDATE storageSpots
        SET {field}='{new_value}'
        WHERE storage_spot = 1;
        '''
        self._execute(
            f'''
            UPDATE {table_name}
            SET {data["field"]} = '{data["new_value"]}'
            WHERE storage_spot = '{data["storage_spot"]}';
            ''',
        )
